...
const AdmissionDocumentType = require("@MEModels/admissionDocumentType");
...
const isActiveAdmissionDocumentTypeExistsValidator = async (value) => {
  const admissionDocumentTypeExists = await AdmissionDocumentType.findById(
    value
  );
  return !!(
    admissionDocumentTypeExists && admissionDocumentTypeExists.is_active
  );
};
