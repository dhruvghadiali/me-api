...
const organizationMemberPostReqBodyValidationSchema = Joi.object({
...
const validateAddOrganizationMemberPostReqBody = asyncHandler(
  async (req, res, next) => {
    try {
      await organizationMemberPostReqBodyValidationSchema.validateAsync(
        req.body,
      );
      next();
    } catch (err) {
